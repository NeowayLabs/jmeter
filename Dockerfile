...
RUN apk upgrade --update && \
    ...
    curl http://mirror.nbtelecom.com.br/apache//jmeter/binaries/apache-jmeter-${JMETER_VERSION}.tgz -o /tmp/jmeter.tar.gz && \
    gunzip /tmp/jmeter.tar.gz && \
    tar -C /opt -xf /tmp/jmeter.tar && \
    ln -s /opt/apache-jmeter-${JMETER_VERSION} ${JMETER_HOME} && \
    mkdir /tmp/plugins && \
    curl https://jmeter-plugins.org/files/packages/jpgc-casutg-2.1.zip -o ${JMETER_HOME}/customThreadGroups.zip && \
    cd ${JMETER_HOME} && \
    unzip customThreadGroups.zip && \
    rm -rf customThreadGroups.zip && \
